  ...
  const { data } = useSystemService(
    (status) =>
      (status as unknown as Record<string, unknown>)
        .claude_usage as ClaudeUsageData | undefined
  );

  if (!data?.available) {
    return (
      <div className="w-fit text-foreground flex items-center gap-2 text-xs opacity-60">
        <span>Claude N/A</span>
      </div>
    );
  }

  const session = data.session_utilization;
  const weekly = data.weekly_utilization;

  return (
    <div className="w-fit text-foreground flex items-center gap-3">
      <ClaudeIcon size={14} />
      <div className="flex items-center gap-1.5">
        <span className="text-xs">
          {formatResetTime(data.session_resets_at)}
        </span>
        <div className="w-20 h-2 rounded-full bg-muted/80 border border-border relative">
          <div
            className={`rounded-full absolute top-0 left-0 h-full ${barColor(session)}`}
            style={{ width: `${Math.min(session, 100)}%` }}
          />
        </div>
      </div>
      <div className="flex items-center gap-1.5">
        <span className="text-xs">
          {formatResetTime(data.weekly_resets_at)}
        </span>
        <div className="w-20 h-2 rounded-full bg-muted/80 border border-border relative">
          <div
            className={`rounded-full absolute top-0 left-0 h-full ${barColor(weekly)}`}
            style={{ width: `${Math.min(weekly, 100)}%` }}
          />
        </div>
      </div>
    </div>
